...
*   **FR-2: User Login:**
    ...
    *   Upon successful login, the system will generate a secure JSON Web Token (JWT) with an appropriate expiration time. This token will be required for all authenticated endpoints.
    *   The JWT payload must include `user_id`, `role`, and `exp` (expiration).

*   **FR-3: User Logout:**
    *   Provide an endpoint to invalidate the user's session.
    *   This will involve blacklisting the JWT in Redis until it expires to prevent reuse.
...
*   **FR-6: Precise Permissions:**
    ...
    *   A dependency injection system in FastAPI will be used to check the JWT and verify the user's role against the required permissions for each endpoint.
...
*   **NFR-1: Security:**
    *   **Authentication:** JWT-based authentication for all protected API routes.
...
*   **NFR-2: Performance:**
    ...
    *   **Caching:** Redis will be used for:
        ...
        *   Storing JWT blacklists for the logout mechanism.
...
*   `POST /auth/login` - Authenticate and receive JWT. **(Requires CSRF token if form-based)**
