    ...
    try:
        result = await db.execute(
            select(User).options(selectinload(
                User.profile)).where(User.email == email)
        )
...
async def get_user_by_public_id(db: AsyncSession, public_id: int) -> Optional[User]:
    ...
    try:
        ...
        logger.error(
            f"Error getting user by public ID: {public_id}", exc_info=True)
        return None
...
async def create_user(
    ...
    try:
        ...
        result = await db.execute(
            select(User).options(selectinload(
                User.profile)).where(User.id == user.id)
        )
...
async def delete_user(db: AsyncSession, email: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Error deleting user enrollments/invoices/payments", exc_info=True)
...
async def list_users(
    ...
    try:
        ...
        if first_name:
            conditions.append(User.profile.has(
                UserProfile.first_name.ilike(f"%{first_name}%")))

        if last_name:
            conditions.append(User.profile.has(
                UserProfile.last_name.ilike(f"%{last_name}%")))

        if current_school:
            conditions.append(User.profile.has(
                UserProfile.current_school.ilike(f"%{current_school}%")))

        if current_grade:
            conditions.append(User.profile.has(
                UserProfile.current_grade.ilike(f"%{current_grade}%")))

        if default_discount_percentage is not None:
            conditions.append(User.profile.has(
                UserProfile.default_discount_percentage > default_discount_percentage))
...
async def create_temporal_user(
    ...
    try:
        ...
        user = User(
            # Temporary email, will be updated
...
async def bulk_create_temporal_users(
    ...
    try:
        ...
        logger.info(
            f"Processing {total_rows} users in {total_batches} batches of {batch_size}")
        ...
        for batch_start in range(0, total_rows, batch_size):
            ...
            logger.info(
                f"Processing batch {batch_num}/{total_batches} (rows {batch_start + 1}-{batch_end})")

            # Process batch
            batch_created, batch_failed = await _process_user_batch_optimized(
                # +2 because CSV starts at row 2 (header is row 1)
                db, batch_rows, batch_start + 2
            )
        ...
        success_rate = (len(created_users) / total_rows) * \
...
async def _process_user_batch_optimized(
    ...
    logger.debug(
        f"Processing batch: {len(rows)} rows starting from row {start_row_num}")
    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            default_discount_percentage = float(
                row.get('default_discount_percentage', '0.0'))
    ...
    logger.debug(
        f"Validation completed in {validation_time:.3f}s: {len(valid_user_data)} valid, {len(failed_rows)} failed")
    ...
    if valid_user_data:
        try:
            ...
            logger.debug(
                f"Starting database operations for {len(valid_user_data)} users")
            ...
            logger.debug(
                f"User flush completed in {flush_time:.3f}s, got {len(users_to_create)} public_ids")
            ...
            for user, user_data in users_to_create:
                ...
                profiles_to_create.append(
                    (profile, user, user_data, temporal_email, temporal_password))

            profile_prep_time = (
                datetime.now() - profile_prep_start).total_seconds()
            logger.debug(
                f"Profile preparation completed in {profile_prep_time:.3f}s")
            ...
            logger.debug(
                f"Database operations completed in {db_time:.3f}s: {len(created_users)} users created")
    ...
    logger.debug(
        f"Batch processing completed in {total_time:.3f}s: {len(created_users)} created, {len(failed_rows)} failed")
